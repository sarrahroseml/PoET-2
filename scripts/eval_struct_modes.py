#!/usr/bin/env python3
"""Evaluate checkpoints with struct + ifq scoring modes.

Runs evaluate_dms_suite with seq_only=False to get per-DMS Spearman
for seq_only, struct, and ifq modes. Reports IID/OOD split.

Each checkpoint is evaluated in a forked subprocess so CUDA errors
from one DMS don't poison the device for subsequent checkpoints.
"""
...
CHECKPOINTS = {
    ...
    "Best_sdiv_mask0": "data/gitignore/checkpoints/sdiv-lr1e-4-mask0p00/best.ckpt",
}
...
def _eval_one_checkpoint(run_name, ckpt, dms_dir, result_path):
    """Run in a subprocess — evaluate one checkpoint and save results to JSON."""
    import gc
    import os
    import torch
    ...
    # Serialize results (convert any non-JSON-safe values)
    safe = {}
...
def main():
    ...
    for run_name, ckpt in CHECKPOINTS.items():
        result_path = os.path.join(save_dir, f"eval_struct_{run_name.replace(' ', '_')}.json")
    ...
    # Collect DMS names (entries that are dicts with spearman keys)
    ...
    # Save JSON
    out = "data/gitignore/struct_eval_results.json"
